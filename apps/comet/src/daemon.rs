...
const LAUNCHD_LABEL: &str = "sh.comet.app";
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Comet headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// The ExecStart binary path. An exe under `~/.comet/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".comet/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.comet/app/current/comet".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.comet/env"));
    ...
    fn curl_installer_always_starts_the_local_capable_service() {
        ...
        assert!(installer.contains("systemctl --user enable comet"));
        assert!(installer.contains("systemctl --user restart comet"));
    }
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.comet/app/0.3.0/comet"),
                Some(Path::new("/home/u")),
            ),
            "%h/.comet/app/current/comet"
        );
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            ...
            Path::new("/Users/x/.comet/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>sh.comet.app</string>"));
        ...
        assert!(
            plist.contains("<key>StandardOutPath</key><string>/Users/x/.comet/daemon.log</string>")
        );
    }
}

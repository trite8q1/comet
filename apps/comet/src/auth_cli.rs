...
use comet_engine::{AuthState, Engine, EngineConfig, InstanceLock, WorkspaceScope};
    ...
    match scope {
        ...
        WorkspaceScope::Synced => match auth {
            ...
            AuthState::NeedsOrganization { user } => AccountStatus {
                mode: "synced",
                auth: format!(
                    "signed in as {} but no workspace selected — run `comet login`",
                    user.email
                ),
                healthy: false,
            },
            AuthState::SignedOut => AccountStatus {
                mode: "synced",
                auth: "saved session is no longer valid — run `comet login`".into(),
                healthy: false,
            },
        },
    }
...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    match auth.state() {
        AuthState::SignedIn { user, org_id } => {
            ...
            println!(
                "Sync is ready. Start or restart Comet to open the synced workspace; existing local sessions will stay local."
            );
        }
...
/// `comet status`: report the fixed scope a new engine would select, optional
...
async fn live_engine_scope(ipc_port: u16) -> Option<WorkspaceScope> {
    let client = comet_rpc::connect_ws(&format!("ws://127.0.0.1:{ipc_port}"))
        ...
        .ok()?;
    let value = client
        .call(comet_rpc::methods::ENGINE_INFO, serde_json::json!({}))
        .await
        .ok()?;
    serde_json::from_value::<comet_engine::EngineInfo>(value)
        .ok()
...
mod tests {
    use super::*;
    use comet_engine::{AuthUser, HarnessId};

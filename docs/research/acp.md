...
- The `npx -y <pinned>` fallback put every user's npm state in the chat hot
  ...
  of report (cometsh/comet#95): npm encodes fatal fs errors as `256 - errno`
...
- Replaced by `adapter_install`: pinned packages install ONCE into
  `~/.comet/adapters/<pkg>/<version>` (`$COMET_ADAPTERS_DIR` overrides) with
  a comet-owned npm cache beside them, atomically (tmp dir + bin-entry
  verification + `.comet-install-ok` marker + rename), then every launch is

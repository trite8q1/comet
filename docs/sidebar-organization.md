...
| 2026-09-01 | Activity hold, folder level only (ADR 0004): under Sort = Last updated, folders active within a 4h window form a stable block ordered by entry (newest entrant inserted at the top); dormant folders stay a recency feed; "No project" pinned last. Activity = persisted messages or a live Working/AwaitingInput session; selection and composer picks never count. Chats inside folders stay pure recency, byte-identical to main; attention-based positioning explicitly rejected (the documented "rows jump when opened" trap). Session-transient, no separator for now. |
| 2026-09-01 | Hold window made configurable (Arc-style): view-options menu section "Active for" with 2 h / 4 h / 8 h / 12 h / 1 day, persisted as `sidebarHoldWindow`, default 4 h. |

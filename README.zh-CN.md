...
![Comet 驱动一个 Claude Code 会话，侧边栏是实时的分支 diff](apps/landing/public/assets/app-screenshot.jpg)
...
```bash
comet status      # 查看本地/同步模式和引擎状态
comet update      # 更新到最新版本
...
如果有引擎正占着数据目录，`comet login` 和 `comet logout` 会拒绝改动凭据。桌面应用同样遵守这条边界：profile 要等下次重启才切换。

macOS 上用桌面版发行包，或者从源码构建 `comet`，再运行 `comet daemon install` 装上 launchd 服务。

---

想参与开发，或者好奇它怎么跑起来的？[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/cometsh/comet)，也可以看 [ARCHITECTURE.md](ARCHITECTURE.md)。

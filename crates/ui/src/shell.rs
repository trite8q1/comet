...
/// `linux_left_captions` is the number of caption buttons comet draws at the
...
pub struct Shell {
    ...
    /// Dev/testing knobs (`COMET_OPEN_DIALOG`, `COMET_FORCE_GATE`,
    /// `COMET_DEMO_UPLOAD`) — see [`Shell::new`].
    ...
    /// The caption buttons comet itself draws on Linux under client-side
...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
                    ...
                    let live = {
                        ...
                        s.selected_chat
                            ...
                            || matches!(
                                ...
                                comet_proto::ConnectivityState::Offline
                                    | comet_proto::ConnectivityState::Reconnecting
                            )
                    };
                    if live {
                        cx.notify();
        ...
        // `COMET_DEMO_UPLOAD=<pct>:<image path>` fabricates an in-flight image
        ...
        let debug_upload = std::env::var("COMET_DEMO_UPLOAD").ok();
    ...
    fn on_state_changed(&mut self, state: &Entity<AppState>, cx: &mut Context<Self>) {
        ...
        // Capture knob: `COMET_DEMO_UPLOAD=<pct>:<image path>` — once a chat
            ...
            && let Some(chat_id) = state.read(cx).selected_chat.clone()
        {
                ...
                && let Ok(att) = crate::attachments::stage_file(std::path::Path::new(img_path))
            {
                ...
                let echo = comet_doc::SessionMessageEntry {
    ...
    fn spawn_subagent_snapshot_fetch(
        ...
        Some(cx.spawn(async move |_, cx| {
            ...
            .await;
            let entries: Option<Vec<comet_doc::SessionMessageEntry>> = reply.ok().and_then(|v| {
    ...
    fn copy_comet_conversation_link(&mut self, chat_id: &str, cx: &mut Context<Self>) {
        let link = {
            ...
            .map(|workspace| crate::links::comet_conversation_link(chat_id, &workspace))
        };
        if let Some(link) = link {
            cx.write_to_clipboard(ClipboardItem::new_string(link));
            self.sidebar_notice = Some("Comet conversation link copied".into());
        } else {
    ...
    /// Which caption buttons comet itself must draw on Linux: under
    ...
    /// controls (native Windows cluster / comet-drawn Linux buttons).
    ...
    /// Comet-drawn Linux caption controls, one overlay per populated side.
    ...
    fn render_sidebar(&mut self, cx: &mut Context<Self>) -> AnyElement {
        // The sidebar is part of the resolved theme. A second fixed-Comet
    ...
    fn render_chat_row(
        ...
        let working = status == comet_proto::ChatIndicator::Working && !queued && !undelivered;
    ...
    fn render_connection_pill(&self, theme: &Theme, cx: &mut Context<Self>) -> Option<AnyElement> {
        use comet_proto::ConnectivityState as S;
    ...
    /// Fetch the manifest and stage the new Comet desktop bundle under the data dir
            ...
            SyncFlow::SwitchOffer { notice_open: true } => {
                ...
                let body: SharedString = match (&signed_in_email, &work_phrase) {
                    ...
                    (Some(email), None) => format!(
                        "You're signed in as {email}. Comet can switch to your synced workspace now."
                    )
                    .into(),
                    ...
                    .into(),
                    (None, None) => "Comet can switch to your synced workspace now.".into(),
                };
                let mut actions = div()
                    .mt(px(16.0))
                    .flex()
        ...
        if let Some(menu_state) = self.chat_menu.get().cloned() {
            ...
            let menu = match menu_state.page {
                ...
                ChatMenuPage::Copy => {
                    ...
                    .child(popover::menu_separator())
                    .child(
                        popover::menu_row(&theme, false, format!("chat-copy-comet-{chat_id}"))
                            .id("chat-copy-comet")
                            .on_click(cx.listener(move |this, _, _, cx| {
                                this.copy_comet_conversation_link(&comet_id, cx)
                            }))
                            .child(
                                icon(icons::COPY)
                                    .size(px(16.0))
                            ...
                            .child(SharedString::from("Comet conversation link")),
                    )
            ...
            let subagent_running = match surface {
                RightSurface::Subagent(id) => self.subagent_tabs.get(&id).is_some_and(|tab| {
                    self.state
                        ...
                        .last()
                        .is_some_and(|e| e.status == Some(comet_doc::MessageStatus::Streaming))
                }),
                _ => false,
            };
...
/// cluster, or past comet's own Linux caption buttons (10px edge inset +
...
/// A Linux caption button in comet's own cluster style (24px, rounded-6,

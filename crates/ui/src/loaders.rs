...
use crate::motion::{self, GRADIENT_SPIN, PULSE_STAGGER, SPLASH_OUT, COMET_PULSE};
...
pub use comet_proto::motion::{
    MARK_CELLS, MARK_SPREAD, MATRIX_SIDE, COMET_CELLS, mark_cell_stagger,
};
    ...
    let content = div()
        ...
        .child(
            div()
                ...
                .child(SharedString::from("Setting up Comet environment")),
        );
    if fading {

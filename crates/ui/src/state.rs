...
pub struct EngineBootConfig {
    /// Data directory for the embedded engine (`~/.comet`).
...
pub struct AppState {
    ...
    pub connectivity: comet_proto::Connectivity,
...
impl AppState {
    pub fn new() -> Self {
        Self {
            ...
            auth: None,
            devices: Vec::new(),
            connectivity: comet_proto::Connectivity::default(),
            spaces: Vec::new(),
            chats: Vec::new(),
            sessions: Vec::new(),
            selected_space: None,
    ...
    pub fn apply_connectivity(&mut self, connectivity: comet_proto::Connectivity) {
    ...
    pub fn apply_transfers(&mut self, transfers: Vec<comet_proto::TransferProgress>) {
    ...
    pub fn open_deep_link(&mut self, url: &str, cx: &mut Context<Self>) {
        match crate::links::parse_comet_conversation_link(url) {
            Ok(link) => {
...
pub use comet_proto::version_triple;
            ...
            while let Some(value) = rx.recv().await {
                ...
                let alive = this.update(cx, |state, cx| {
                    ...
                    if let Some(rows) = state.sub_transcripts.get_mut(&doc_id) {
                        if let Err(err) = comet_doc::apply_transcript_frame(rows, frame) {
...
mod tests {
    ...
    fn transfer_percent_tracks_snapshots_by_upload_id() {
        ...
        let frame = |id: &str, done, total| comet_proto::TransferProgress {
            upload_id: id.into(),
    ...
    fn delivery_degradation_and_queued_sends_tell_the_truth() {
        use comet_proto::{ChatConnectivity, ConnectivityState};
        let now = Utc::now();
        let mut s = AppState::default();

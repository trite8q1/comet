...
//! - **Managed** (`~/.comet/app/<ver>` + `current` symlink — the curl|sh
...
pub enum InstallKind {
    /// `~/.comet/app/<ver>/comet` behind the `current` symlink
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".comet").join("app");
...
pub fn restart_service() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/sh.comet.app")],
        )
    } else {
        run("systemctl", &["--user", "restart", "comet.service"])
    }
}
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.comet/app/0.1.1/comet"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.comet/app")
            }
        );
        assert_eq!(
            detect_install_from(

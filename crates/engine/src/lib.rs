...
pub struct EngineConfig {
    /// Data directory (default `~/.comet`, dev `~/.comet-dev`).
...
impl Engine {
    ...
    async fn assemble_runtime_inner(
        ...
        if edge_enabled {
            ...
            comet_sync::net_path::spawn_path_monitor();
        }
        ...
        comet_harness::acp::prewarm_managed_adapters();

        let host_relay = edge.as_ref().map(|edge| {
            let mut link_config =
                comet_rpc::LinkCacheConfig::new(edge.url.clone(), Arc::new(auth.clone()));
            ...
            let links = comet_rpc::LinkCache::new(link_config);

...
struct RuntimeConfig {
    ...
    auto_approve: bool,
    worktree: Option<comet_proto::WorktreeSpec>,
}
...
impl SubagentSink {
    ...
    fn finish(mut self, device_id: &str, status: MessageStatus) -> Option<String> {
        ...
        let entries = comet_doc::join_continuation_entries(self.doc.read_entries().ok()?);
    ...
    let final_status = loop {
        ...
        } = &event
        {
            ...
            if chip_streaming {
                ...
                comet_doc::fold_event_into_parts(&mut folded, &event);
                if !dirty {
                    dirty = true;
            ...
            if let Some(sink) = subagents.get_mut(parent_tool_use_id) {
                ...
                comet_doc::fold_event_into_parts(&mut sink.folded, sub_event);
                sink.dirty = true;
...
mod tests {
    ...
    use comet_proto::{HarnessId, RunRequest, SandboxLevel};
    ...
    fn live_routing_requires_the_same_runtime_configuration() {
        ...
        follow_up.reasoning = Some(comet_proto::ReasoningLevel::Medium);
        assert!(!config.can_route(HarnessId::Grok, &follow_up));

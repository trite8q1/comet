...
use comet_proto::{AgentEvent, SUBAGENT_INPUT_KEEP, ToolCall, ToolDiff, UserInputQuestion};
...
pub fn fold_event_into_parts(out: &mut Vec<MessagePart>, event: &AgentEvent) {
    match event {
        ...
        AgentEvent::Subagent {
            ...
            let status = match event.as_ref() {
                AgentEvent::Done { status, .. } => Some(match status {
                    comet_proto::DoneStatus::Errored => SubagentStatus::Failed,
        ...
        // AvailableCommands is retired (§10.4) and reaches the fold only from
        // a pre-retirement run journal; it was never transcript content.
        // UserMessage becomes its own doc ENTRY (the engine's subagent sink
        // writes it), never a part of the assistant message.

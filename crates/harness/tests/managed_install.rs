...
//! `$COMET_ADAPTERS_DIR`, spawn it via node, and reach SessionStarted (the
//! full initialize → session/new handshake) — the exact path that used to be
//! `npx -y` at chat time (cometsh/comet#95).
//!
//! Ignored: needs network, npm, and the pi CLI on the machine. Run with
//! `cargo test -p comet-harness --test managed_install -- --ignored`.
//!
...
async fn managed_install_reaches_session_started() {
    ...
    unsafe {
        std::env::set_var("COMET_ADAPTERS_DIR", adapters.path());
    ...
    let version_dir = std::fs::read_dir(&root)
        ...
        .path();
    assert!(version_dir.join(".comet-install-ok").exists());
}

//! The cometsh/comet#95 condition, reproduced: npm dying silently with an
...
//!
//! Single-test binary: it mutates PATH/SHELL/COMET_* env process-wide.
...
async fn silent_npm_enoent_death_surfaces_decoded_error() {
    ...
    unsafe {
        std::env::set_var("COMET_ADAPTERS_DIR", dir.path().join("adapters"));

...
//! (254 = ENOENT, the cometsh/comet#95 crash) that surfaced as an opaque
//! "harness protocol error". Instead, pinned adapter packages are installed
//! ONCE into a comet-owned prefix (`~/.comet/adapters/<pkg>/<version>`, own
...
const OK_MARKER: &str = ".comet-install-ok";
const INSTALL_TIMEOUT: Duration = Duration::from_secs(600);

/// `$COMET_ADAPTERS_DIR`, else `~/.comet/adapters`.
fn adapters_root() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("COMET_ADAPTERS_DIR").filter(|d| !d.is_empty()) {
        return Some(PathBuf::from(dir));
    }
    std::env::var_os("HOME")
        .filter(|h| !h.is_empty())
        .map(|h| PathBuf::from(h).join(".comet").join("adapters"))
}
...
/// A comet-owned shim script materialized INSIDE a managed install dir, for
...
/// comet-owned shim rather than through a bin entry. Installs the pin once,

...
//! `@cursor/sdk` via a thin comet-owned Node shim (`shim.mjs`, JSONL over
...
//!   (`~/.comet/adapters/…`, [`crate::adapter_install::ensure_installed_shim`])
...
impl Harness for CursorHarness {
    ...
    /// install comet performs on demand).
            ...
            line = stdout_lines.next_line() => match line {
                Ok(Some(line)) => {
                    ...
                    let Ok(frame) = serde_json::from_str::<Value>(line) else {
                        tracing::debug!(target: "comet_harness::cursor", "unparseable shim frame (skipped)");
                        continue;
                    };
...
fn map_shim_frame(frame: &Value, interrupted: bool) -> Vec<AgentEvent> {
    ...
    match frame.get("ev").and_then(Value::as_str).unwrap_or("") {
        ...
        other => {
            tracing::debug!(target: "comet_harness::cursor", "unknown shim frame (skipped): {other}");
            Vec::new()
        }
    }
}
